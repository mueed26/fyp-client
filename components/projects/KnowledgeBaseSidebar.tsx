...
const documentUtils = {
  ...
  getStatusText: (status: string) => {
    const texts: { [key: string]: string } = {
      uploading: "Uploading", queued: "Queued", partitioning: "Processing",
      chunking: "Chunking", summarising: "Summarising", vectorization: "Vectorizing",
...
const SliderField = ({ label, value, min, max, step = 1, onChange, disabled, info }: {
...
export function KnowledgeBaseSidebar({
  ...
  const getPerformanceMetrics = () => {
    ...
    const strategyConfig = {
      basic: { latency: 400 }, hybrid: { latency: 600 },
      "multi-query-vector": { latency: 800 }, "multi-query-hybrid": { latency: 1000 },
    ...
    return { totalChunks, latency: baseLatency + queryLatency + rerankingLatency };
  };
        ...
        {tabs.map((tab) => (
          <button
            ...
            onClick={() => {
              ...
              onSetActiveTab(tab.id as "documents" | "settings" | "studio");
            }}
        ...
        {activeTab === "documents" && (
          <div className="p-6 space-y-8 bg-[#1a1a1a] text-white h-full overflow-y-auto">
            <section className="space-y-6">
              ...
              <div
                {...getRootProps()}
                className={`border border-dashed rounded-lg p-6 text-center transition-colors cursor-pointer ${isDragActive ? "border-gray-600 bg-[#252525]" : "border-gray-700 hover:border-gray-600 bg-[#202020] hover:bg-[#252525]"
                  }`}
              >
                <input {...getInputProps()} />
                <div className="flex flex-col items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-400 mt-1">
                      {isDragActive ? "Release to upload" : "PDF, DOCX, PPT, MD, TXT • Max 50MB"}
                    </p>
                  </div>
                </div>
              </div>
                            ...
                            <div className="flex items-center justify-between mt-1">
                              ...
                              {doc.processing_status && doc.processing_status !== "completed" && (
                                <div className="flex items-center gap-1 text-xs text-gray-400">
                                  ...
                                  <span>{documentUtils.getStatusText(doc.processing_status)}</span>
                                </div>
                              )}
                            </div>
                          </div>
              ...
              <div className={`space-y-8 ${settingsLoading ? "opacity-50 pointer-events-none" : ""}`}>
                <section className="space-y-4">
                  <div className="flex items-center gap-2">
                    <h3 className="text-sm font-medium text-gray-200">Embedding Model</h3>
                    <div className="w-3 h-3 bg-amber-500/20 border border-amber-500/20 rounded-full flex items-center justify-center"
                      title={isEmbeddingLocked ? "Locked (documents uploaded)" : "Locked after first document upload"}>
                  ...
                  <p className="text-xs text-amber-400/60">
                    {isEmbeddingLocked ? "Locked (documents uploaded)" : "Locked after first document upload"}
                  </p>
                </section>
                ...
                <section className="space-y-4">
                  ...
                  <div className="space-y-2">
                    {STRATEGY_OPTIONS.map((strategy) => (
                      <label
                        key={strategy.value}
                        className={`block p-3 rounded-lg border cursor-pointer transition-colors ${projectSettings.rag_strategy === strategy.value ? "border-gray-600 bg-[#252525]" : "border-gray-800 bg-[#202020] hover:border-gray-700"
                          }`}
                      >
                        <div className="flex items-center gap-3">
                          <input type="radio" name="ragStrategy" value={strategy.value}
                ...
                <section className="space-y-5">
                  ...
                  <SliderField label="Chunks per Search" value={projectSettings.chunks_per_search} min={5} max={30}
                    onChange={(e) => onUpdateSettings({ chunks_per_search: parseInt(e.target.value) })} disabled={settingsLoading} />
                  <SliderField label="Final Context Size" value={projectSettings.final_context_size} min={3} max={10}
                    onChange={(e) => onUpdateSettings({ final_context_size: parseInt(e.target.value) })} disabled={settingsLoading} />
                  <SliderField label="Similarity Threshold" value={projectSettings.similarity_threshold} min={0.1} max={0.9} step={0.1}
                    onChange={(e) => onUpdateSettings({ similarity_threshold: parseFloat(e.target.value) })} disabled={settingsLoading} />
                  {isMultiQuery && (
                    <div className="pt-2 border-t border-gray-800">
                      <SliderField label="Number of Queries" value={projectSettings.number_of_queries} min={3} max={7}
                        onChange={(e) => onUpdateSettings({ number_of_queries: parseInt(e.target.value) })} disabled={settingsLoading} />
                    </div>
                  )}
                </section>
                ...
                {isHybrid && (
                  <>
                    <hr className="border-gray-800" />
                    <section className="space-y-4">
                      <h3 className="text-sm font-medium text-gray-200">Search Weights</h3>
                      <SliderField label="Vector Weight" value={projectSettings.vector_weight} min={0.1} max={0.9} step={0.1}
                        onChange={(e) => { const v = parseFloat(e.target.value); onUpdateSettings({ vector_weight: v, keyword_weight: 1 - v }); }}
                        disabled={settingsLoading}
                ...
                <section className="space-y-4">
                  ...
                  <label className="flex items-center gap-3 cursor-pointer">
                    <input type="checkbox" checked={projectSettings.reranking_enabled}
                    ...
                    <span className="text-sm text-gray-200">Enable reranking</span>
                  </label>
                  {projectSettings.reranking_enabled && (
                    <div className="ml-7 space-y-2">
                      <label className="text-xs text-gray-400">Model</label>
                      <select value={projectSettings.reranking_model}
                        ...
                        {RERANKING_MODELS.map((model) => (<option key={model.value} value={model.value}>{model.label}</option>))}
                      </select>
                    </div>
                  )}
                </section>
                ...
                <section className="space-y-4">
                  ...
                  <div className="space-y-2">
                    {AGENT_MODE_OPTIONS.map((mode) => (
                      <label
                        key={mode.value}
                        className={`block p-3 rounded-lg border cursor-pointer transition-colors ${projectSettings.agent_type === mode.value ? "border-gray-600 bg-[#252525]" : "border-gray-800 bg-[#202020] hover:border-gray-700"
                          }`}
                      >
                        <div className="flex items-center gap-3">
                          <input type="radio" name="agentMode" value={mode.value}
                  ...
                  <div className="bg-[#252525] border border-gray-700 rounded-lg p-4">
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      <div className="text-center">
                        <div className="text-lg font-medium text-gray-100">~{getPerformanceMetrics().totalChunks}</div>
                        <div className="text-xs text-gray-400">Total chunks</div>
                      </div>
                      <div className="text-center">
                        <div className="text-lg font-medium text-gray-100">~{getPerformanceMetrics().latency}ms</div>
                ...
                <button onClick={onApplySettings} disabled={settingsLoading}
